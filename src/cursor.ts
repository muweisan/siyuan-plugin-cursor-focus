...
export class CursorFocus {
  ...
  /** 从插件数据中读取设置（带默认值兜底） */
  get settings() {
    const data = (this.plugin.data ?? {}) as Record<string, unknown>;
    return {
      autoFocus: typeof data.autoFocus === "boolean" ? data.autoFocus : true,
      highlightDuration:
        typeof data.highlightDuration === "number" ? data.highlightDuration : 2000,
    };
  }
   ...
   */
  async focusToCursor(editor?: HTMLElement) {
    try {
      ...
      if (!block) {
        this.showMessage(
          this.plugin.i18n.noCursor ?? "当前文档没有光标，无法定位",
          3000,
          "info"
        );
        return;
  ...
  /**
   * 显示消息提示
   * 兼容新版思源（独立导出的 showMessage 函数）与旧版（Plugin#showMessage 方法）
   */
  private showMessage(text: string, timeout: number, type: "info" | "error") {
    const legacyPlugin = this.plugin as unknown as {
      showMessage?: (t: string, tm?: number, ty?: "info" | "error") => void;
    };
    if (typeof legacyPlugin.showMessage === "function") {
      legacyPlugin.showMessage(text, timeout, type);
    } else {
  ...
  /** 兜底监听文档切换（指南中的 MutationObserver 方案） */
